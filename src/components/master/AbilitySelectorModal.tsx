...
export const AbilitySelectorModal: React.FC<AbilitySelectorModalProps> = ({ isOpen, onClose, onSelect }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="bg-zinc-950 border border-zinc-800 rounded-xl w-full max-w-2xl max-h-[80vh] flex flex-col shadow-2xl animate-in fade-in zoom-in-95 duration-200">
        
        <div className="p-6 border-b border-zinc-800 flex justify-between items-center bg-zinc-900/50">
          <div>
            <h3 className="text-xl font-bold text-white">Adicionar Habilidade</h3>
            <p className="text-zinc-400 text-sm">Selecione um poder ou habilidade para adicionar à ficha.</p>
          </div>
        ...
        <div className="p-4 border-b border-zinc-800 bg-zinc-900/30 flex gap-4">
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full bg-zinc-950 border border-zinc-800 rounded-lg pl-10 pr-4 py-2 text-zinc-200 focus:outline-none focus:border-zinc-600 transition-colors"
              autoFocus
            />
          </div>
          ...
          <select 
            ...
            className="bg-zinc-950 border border-zinc-800 rounded-lg px-4 py-2 text-zinc-300 focus:outline-none focus:border-zinc-600"
          >
            ...
            {filteredAbilities.length === 0 ? (
                ...
                filteredAbilities.map((ability, idx) => (
                    <button 
                        ...
                        className="w-full text-left bg-zinc-900/30 border border-zinc-800 rounded-lg p-4 hover:bg-zinc-900 hover:border-zinc-600 transition-all group"
                    >
                        <div className="flex justify-between items-start mb-1">
                            <h4 className="font-bold text-zinc-200 group-hover:text-white">{ability.nome}</h4>
                            <span className="text-xs font-mono bg-black/50 px-2 py-0.5 rounded text-zinc-500 border border-zinc-800 uppercase">
                                {ability.tipo}
                            </span>
                        </div>
                        ...
                        {(ability.custo || ability.acao) && (
                            <div className="flex gap-3 mt-2 text-xs font-mono text-zinc-500">
