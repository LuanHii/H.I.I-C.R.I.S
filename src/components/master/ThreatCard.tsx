...
export const ThreatCard: React.FC<ThreatCardProps> = ({ threat, onUpdateHP, onRemove }) => {
  ...
  return (
    <div className={`bg-zinc-950/50 border ${isExpanded ? 'border-zinc-600' : 'border-zinc-700'} rounded-lg overflow-hidden transition-all duration-200`}>
      <div className="p-4">
        <div className="flex justify-between items-start mb-3">
          <div className="flex-1 cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
            <div className="flex items-center gap-2">
                ...
                {isExpanded ? (
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-zinc-500"><path d="m18 15-6-6-6 6"/></svg>
                ) : (
            ...
            <div className="flex gap-2 mt-1">
              <span className="text-[10px] font-mono bg-black/50 px-1.5 py-0.5 rounded text-zinc-400 border border-zinc-700">VD {data.vd}</span>
              <span className="text-[10px] font-mono bg-black/50 px-1.5 py-0.5 rounded text-zinc-400 border border-zinc-700">DEF {data.defesa}</span>
              <span className="text-[10px] font-mono bg-black/50 px-1.5 py-0.5 rounded text-zinc-400 border border-zinc-700">{data.tamanho}</span>
            </div>
          </div>
          <button 
            onClick={() => onRemove(threat.instanceId)}
            className="text-zinc-500 hover:text-red-400 transition-colors p-1 ml-2"
          >
        ...
        <div className="mt-2">
          <div className="flex justify-between text-xs mb-1 font-mono">
            <span className="text-zinc-400">PV</span>
            <span className="text-zinc-200">{currentHP} / {data.vida}</span>
          </div>
          <div className="h-3 bg-zinc-950 border border-zinc-700 rounded-full overflow-hidden relative">
            <div 
      ...
      {isExpanded && (
        <div className="border-t border-zinc-700 bg-zinc-900/30 p-4 animate-in slide-in-from-top-2">
            
            <div className="flex items-center justify-between mb-6 bg-zinc-950 p-2 rounded border border-zinc-800">
                <span className="text-xs font-mono text-zinc-500 uppercase ml-2">Gerenciar Vida</span>
            ...
            <div className="flex justify-between gap-2 mb-6">
                {Object.entries(data.atributos).map(([key, val]) => (
                    <div key={key} className="flex flex-col items-center flex-1 bg-zinc-950/50 p-2 rounded border border-zinc-800">
                        <span className="text-[10px] font-mono text-zinc-500 uppercase">{key}</span>
                        <span className="text-lg font-bold text-zinc-200">{val}</span>
                    </div>
                ))}
            </div>
                ...
                <div className="mb-6 space-y-2 text-xs">
                    {data.resistencias && data.resistencias.length > 0 && (
                        <div className="flex gap-2">
                            <span className="text-zinc-500 font-bold">RESISTÊNCIAS:</span>
                            <span className="text-zinc-300">{data.resistencias.join(', ')}</span>
                        </div>
                    )}
                    {data.vulnerabilidades && data.vulnerabilidades.length > 0 && (
                        <div className="flex gap-2">
                            <span className="text-zinc-500 font-bold">VULNERABILIDADES:</span>
                            <span className="text-zinc-300">{data.vulnerabilidades.join(', ')}</span>
                        </div>
                    )}
                    {data.imunidades && data.imunidades.length > 0 && (
                        <div className="flex gap-2">
                            <span className="text-zinc-500 font-bold">IMUNIDADES:</span>
                            <span className="text-zinc-300">{data.imunidades.join(', ')}</span>
                        </div>
                    )}
                </div>
            )}
            ...
            {data.acoes.length > 0 && (
                <div className="mb-6">
                    <h5 className="text-xs font-bold text-zinc-500 uppercase mb-3 border-b border-zinc-800 pb-1">Ações</h5>
                    <div className="space-y-3">
                        {data.acoes.map((acao, idx) => (
                            <div key={idx} className="bg-zinc-950/30 p-3 rounded border border-zinc-800">
                                <div className="flex justify-between items-start mb-1">
                                    <span className="font-bold text-zinc-200 text-sm">{acao.nome}</span>
                                    {acao.tipo && <span className="text-[10px] bg-zinc-900 px-1.5 py-0.5 rounded text-zinc-500 border border-zinc-800">{acao.tipo}</span>}
                                </div>
                                <p className="text-xs text-zinc-400 leading-relaxed">{acao.descricao}</p>
                                {(acao.teste || acao.dano) && (
                                    <div className="flex gap-3 mt-2 text-[10px] font-mono text-zinc-500">
                                        {acao.teste && <span className="text-zinc-300">Teste: {acao.teste}</span>}
            ...
            {data.habilidades.length > 0 && (
                <div>
                    <h5 className="text-xs font-bold text-zinc-500 uppercase mb-3 border-b border-zinc-800 pb-1">Habilidades</h5>
                    <div className="space-y-3">
                        {data.habilidades.map((hab, idx) => (
                            <div key={idx} className="bg-zinc-950/30 p-3 rounded border border-zinc-800">
                                <span className="font-bold text-zinc-200 text-sm block mb-1">{hab.nome}</span>
                                <p className="text-xs text-zinc-400 leading-relaxed">{hab.descricao}</p>
                            </div>
                        ))}
                    </div>
                </div>
