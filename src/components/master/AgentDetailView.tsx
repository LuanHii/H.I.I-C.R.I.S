...
export const AgentDetailView: React.FC<AgentDetailViewProps> = ({ agent, onUpdate, readOnly }) => {
  ...
  const handleAddItem = (item: Item) => {
    ...
    // Recalculate load if needed, but for now just add
        ...
        <div className="relative z-10 flex justify-between items-start">
            <div>
                ...
                <div className="flex items-center gap-3 text-zinc-300 font-mono text-sm">
                    ...
                    <span className="bg-zinc-800 px-2 py-1 rounded border border-zinc-600">{agent.nex}% NEX</span>
            ...
            <div className="p-6 animate-in slide-in-from-top-2">
                <div className="flex justify-between gap-4 mb-8">
                    {Object.entries(agent.atributos).map(([key, val]) => (
                        <div key={key} className="flex flex-col items-center flex-1 bg-zinc-950/50 p-3 rounded border border-zinc-700">
                ...
                <div className="space-y-2">
                    {agent.equipamentos.map((item, idx) => (
                        <div key={idx} className="flex items-center justify-between p-3 bg-zinc-950/50 rounded border border-zinc-700 hover:border-zinc-500 group">
                            ...
                            {!readOnly && (
                            <button className="opacity-0 group-hover:opacity-100 p-2 hover:bg-zinc-800 rounded text-zinc-400 hover:text-white transition-all">
        ...
        {openSections['abilities'] && (
            <div className="p-6 animate-in slide-in-from-top-2 space-y-4">
                {agent.poderes.map((poder, idx) => (
                    <div key={idx} className="bg-zinc-950/50 p-4 rounded border border-zinc-700">
